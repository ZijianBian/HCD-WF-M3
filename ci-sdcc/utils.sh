...
getModuleName() {
    ...
    if [[ $MODULE_NAME == *-* ]]; then
        module_versions=$(module -t avail "$MODULE_NAME"/ 2>&1 | grep -E "$MODULE_NAME/[0-9]+\.[0-9]+\.[0-9]+|dev")
    else
        module_versions=$(module -r -t avail ^"$MODULE_NAME"/ 2>&1 | grep -E "$MODULE_NAME/[0-9]+\.[0-9]+\.[0-9]+|dev")
    fi
