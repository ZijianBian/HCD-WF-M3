...
class WorkflowExecutor:
    ...
    def _load_ic_toroidal_modes(self, config_folder_path):
        ...
        modes = []
        for index, entry in enumerate(raw_modes, start=1):
            try:
                n_phi = int(entry["n_phi"])
                weight = float(entry.get("weight", 1.0))
            except (TypeError, ValueError, KeyError) as error:
                raise ValueError(f"Invalid CYRANO mode {index} in {path}") from error
            if not math.isfinite(weight) or weight < 0.0:
                raise ValueError(f"CYRANO mode {index} weight must be finite and nonnegative")
            if weight > 0.0:
                modes.append({"n_phi": n_phi, "weight": weight})

        total_weight = sum(mode["weight"] for mode in modes)
        if not math.isfinite(total_weight):
            raise ValueError("CYRANO mode weights overflow their total")
        if total_weight <= 0.0:
            return []
        for mode in modes:
