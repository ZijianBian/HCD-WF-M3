...
def parse_workflow_xml(xml_path):
    ...
    params = {
        ...
        'input_backend': wf.find('input_backend').text,
        'ddv_backend': wf.find('ddv_backend').text,
            ...
            while timenow < self.tend:
                ...
                try:
                    ...
                    input_equilibrium = imas.convert_ids(input_equilibrium, target_dd_version)
                                  ...
                                  # MESS DUE TO THAT B_FIELD_TOR AND B_FIELD_PHI BOTH EXISTS IN IMAS 3.42
                    if not input_equilibrium.time_slice[0].global_quantities.magnetic_axis.b_field_phi.has_value \
                        and input_equilibrium.time_slice[0].global_quantities.magnetic_axis.b_field_tor.has_value:
                        input_equilibrium.time_slice[0].global_quantities.magnetic_axis.b_field_phi = \
                            input_equilibrium.time_slice[0].global_quantities.magnetic_axis.b_field_tor
                    if not input_equilibrium.time_slice[0].profiles_2d[0].b_field_phi.has_value \
                        and input_equilibrium.time_slice[0].profiles_2d[0].b_field_tor.has_value:
                        input_equilibrium.time_slice[0].profiles_2d[0].b_field_phi = \
                            input_equilibrium.time_slice[0].profiles_2d[0].b_field_tor
