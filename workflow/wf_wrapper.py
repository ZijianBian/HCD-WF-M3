"""
wf_wrapper.py — Unified HCD Workflow Entry Point
...
Architecture:
    m3_flag=0 (Traditional):
        wf_wrapper.py (this file)
            ├── Database I/O
            ├── Time loop
            └── HCDWorkflow.run() directly (iWrap actors in-process)

    m3_flag=1 (MUSCLE3 Hybrid, this file = macro model):
        wf_wrapper.py (this file, MUSCLE3 macro)
            ├── Database I/O
            ├── Time loop (timestep-level reuse)
            └── M3 send/receive IDS ↔ hcdworkflow/m3_micro.py

        hcdworkflow/m3_micro.py (MUSCLE3 micro, separate process)
