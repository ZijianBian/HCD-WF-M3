"""Prepare IDS inputs and stabilize HCD output structures.

Input helpers convert DD versions and fill missing fields required by actors.
Output helpers preserve EC/IC slots across on/off slices to keep the HDF5
structure consistent. Multi-mode CYRANO outputs are combined by power weight.
These helpers operate on IDS contents without opening database connections.
"""
...
def get_backend(backend_name: str):
    ...
    if hasattr(imas, "ids_defs"):
        ...
        raise RuntimeError("Cannot find IMAS backend definitions")


def _get_target_dd_version():
    """Get the DD version of the current IMAS environment."""
    configured_version = os.getenv("IMAS_VERSION")
    if configured_version:
        return configured_version
    if hasattr(imas, "al_dd_version"):
        return imas.al_dd_version
    return imas.dd_zip.dd_xml_versions()[-1]
...
def _smart_convert(ids_object, ids_name):
    ...
    """
    # The generated IMAS-AL API reads directly into its compiled DD. Its fields
    # are plain scalars/arrays, and it has neither convert_ids nor has_value.
...
def _fixup_core_profiles(cp):
    ...
    if n_fixed_temp or n_fixed_density or n_fixed_flag:
        print(
            f"  [core_profiles] Filled ion fields for Cyrano "
            f"(temperature={n_fixed_temp}, density_thermal={n_fixed_density}, "
            f"multiple_states_flag={n_fixed_flag})",
            flush=True,
        )
...
def _eq_fill_psi_axis(ts):
    ...
    print(f"  [equilibrium] Filled psi_axis from profiles_1d.psi[0] " f"(= {psi1d_axis:.4g})", flush=True)
...
def _eq_replace_nan_2d(eq, ts):
    ...
    p2d.b_field_phi[np.isnan(p2d.b_field_phi)] = np.sign(float(eq.vacuum_toroidal_field.b0[0])) * 99.0
...
def _eq_rename_b_field_tor(ts):
    ...
    if hasattr(mag, "b_field_tor") and not mag.b_field_phi.has_value and mag.b_field_tor.has_value:
        print("  [equilibrium] Copying b_field_tor → b_field_phi (magnetic_axis)", flush=True)
        mag.b_field_phi = mag.b_field_tor

    if (
        len(ts.profiles_2d) > 0
        and hasattr(ts.profiles_2d[0], "b_field_tor")
...
def _eq_complete_triangularity(ts):
    ...
    if not ts.profiles_1d.triangularity_lower.has_value and ts.profiles_1d.triangularity_upper.has_value:
        print("  [equilibrium] Copying triangularity_upper → triangularity_lower", flush=True)
        ts.profiles_1d.triangularity_lower = copy.deepcopy(ts.profiles_1d.triangularity_upper)
    ...
    try:
        ...
        print(
            f"  [equilibrium] Computed elongation/triangularity profiles "
            f"(kappa={kappa_edge:.3f}, delta_u={delta_upper_edge:.3f}, "
            f"delta_l={delta_lower_edge:.3f})",
            flush=True,
        )
    except Exception as e:
...
def _eq_compute_r_outboard_inboard(ts):
    ...
    try:
        ...
        f_out = interp1d(psi_mid[ir_axis:], R1d[ir_axis:], bounds_error=False, fill_value=np.nan)
        f_in = interp1d(psi_mid[: ir_axis + 1], R1d[: ir_axis + 1], bounds_error=False, fill_value=np.nan)
        ...
        r_outboard, r_inboard, repaired_axis = _repair_axis_minor_radius(ts, r_outboard, r_inboard)

        ts.profiles_1d.r_outboard = r_outboard
        ts.profiles_1d.r_inboard = r_inboard

        print(
            f"  [equilibrium] Computed r_outboard/r_inboard from 2D psi " f"({len(psi1d)} points, R_axis={r_axis:.3f})",
            flush=True,
        )
        if repaired_axis:
            rho1 = 0.5 * (r_outboard[1] - r_inboard[1])
            print(
                f"  [equilibrium] Repaired first off-axis minor radius " f"for Cyrano (rho[1]={rho1:.4g} m)", flush=True
            )
    except Exception as e:
...
def _eq_fill_rho_tor_from_flux(eq, ts):
    ...
    ITER equilibrium slices provide rho_tor_norm and phi, but leave
    ...
    try:
        ...
        finite_phi = np.isfinite(phi) & (np.abs(phi) < 1.0e35)
        ...
        if (
            ...
            or rho_tor[-1] <= 0.0
            or np.any(np.diff(rho_tor) < -1.0e-8)
        ):
        ...
        p1d.rho_tor = rho_tor
        print(
            f"  [equilibrium] Filled rho_tor from toroidal flux for FoPla "
            f"({rho_tor.size} points, edge={rho_tor[-1]:.4g} m)",
            flush=True,
        )
    except Exception as e:
...
def _update_equilibrium_bfield(eq):
    ...
    if len(ts.profiles_2d) == 0:
        ...
        phi[np.where(phi > 1.0e40)] = np.nan
        ...
        R_2D, Z_2D = np.meshgrid(np.linspace(r_min, r_max, N_grid_r), np.linspace(z_min, z_max, N_grid_z))
        ...
        for field in ["psi", "phi", "b_field_r", "b_field_phi", "b_field_z", "j_phi"]:
            try:
                ...
                setattr(ts.profiles_2d[0], field, scipy_griddata((rr, zz), src_vals, (R_2D, Z_2D)).T)
            except Exception as e:
...
def _mode_stack_field(target, sources, field_name, weights, scale=True):
    ...
    if first.shape == ():
        values = [(float(weight) if scale else 1.0) * float(np.asarray(arr)) for weight, arr in zip(weights, arrays)]
...
def merge_ic_wave_modes(mode_waves, n_phi_values, weights):
    ...
    if not np.isfinite(total_weight) or total_weight <= 0.0:
        raise ValueError("mode weights must sum to a finite positive value")
...
def _n_phi_values(waves=None, config_folder_path=None):
    for wave in getattr(waves, "coherent_wave", []):
        if str(wave.identifier.type.name).lower() == "ec" or int(wave.identifier.type.index) == 1:
            continue
        n_phi = _n_phi_values_for_wave(wave)
...
def _n_phi_values_for_wave(coherent_wave):
    for path in (
        ...
        shape = _wave_array_shape(coherent_wave, path)
        if shape:
            values = getattr(coherent_wave, path[0])[0].n_phi
...
def _fill_zero_ic_wave_payload(coherent_wave, input_slices, source_index, timenow, waves=None, config_folder_path=None):
    radial_shape = _wave_array_shape(coherent_wave, ("profiles_1d", 0, "power_density")) or _wave_array_shape(
        coherent_wave, ("profiles_1d", 0, "grid", "rho_tor_norm")
    )
...
def _fill_zero_wave_slot(
    coherent_wave, source_kind, source_index, input_slices, timenow, waves=None, config_folder_path=None
):
    if source_kind == "ic":
        _set_code(coherent_wave.identifier.type, "IC", 3)
        _set_code(coherent_wave.wave_solver_type, "IC", 2)
        coherent_wave.identifier.antenna_name = _ic_antenna_name(input_slices.get("ic_antennas"), source_index)
    ...
    coherent_wave.identifier.antenna_name = _ec_beam_name(input_slices.get("ec_launchers"), source_index)
...
def _align_wave_slots(waves, expected, input_slices):
    """Place compact actor results in stable EC/IC slots before filling gaps."""
    existing = list(waves.coherent_wave)
    kinds = []
    for wave in existing:
        kind = str(wave.identifier.type.name).strip().lower()
        if kind not in ("ec", "ic"):
            kind = {1: "ec", 3: "ic"}.get(int(wave.identifier.type.index))
        kinds.append(kind)

    remaining = list(range(len(existing)))
    order = []
    expected_names = [
        (
            _ec_beam_name(input_slices.get("ec_launchers"), source_index)
            if kind == "ec"
            else _ic_antenna_name(input_slices.get("ic_antennas"), source_index)
        )
        for kind, source_index in expected
    ]
    for (kind, _), name in zip(expected, expected_names):
        candidates = [idx for idx in remaining if kinds[idx] in (kind, None)]
        named = [idx for idx in candidates if str(existing[idx].identifier.antenna_name) == name]
        # A result named for another expected antenna belongs to that later slot.
        unnamed = [idx for idx in candidates if str(existing[idx].identifier.antenna_name) not in expected_names]
        match = next(iter(named or unnamed), None)
        order.append(match)
        if match is not None:
            remaining.remove(match)

    missing = {idx for idx, source in enumerate(order) if source is None}
    order.extend(remaining)
    if order != list(range(len(existing))):
        # Copy only when slots move: clearing the AoS invalidates its old entries.
        reordered = [copy.deepcopy(existing[idx]) if idx is not None else None for idx in order]
        waves.coherent_wave.resize(0)
        waves.coherent_wave.resize(len(order))
        for idx, wave in enumerate(reordered):
            if wave is not None:
                waves.coherent_wave[idx] = wave
    return missing


def _ensure_waves_placeholders(input_slices, output_ids, param_process, timenow, config_folder_path=None):
    expected = _expected_wave_slots(input_slices, param_process)
    if not expected:
        return

    waves = _ensure_output_slice(output_ids, "waves", timenow)
    missing = _align_wave_slots(waves, expected, input_slices)
    ...
    for slot_index, (source_kind, source_index) in enumerate(expected):
        ...
        if slot_index not in missing and not source_is_inactive:
            continue
...
def _ensure_source_profiles_1d(source, sibling_sources, input_slices, timenow, config_folder_path):
    ...
    """
    nrho = _source_radial_size(source, sibling_sources, input_slices, config_folder_path)
    if nrho <= 0:
        return
...
def _fill_zero_source_slot(
    source, source_kind, timenow, sibling_sources=(), input_slices=None, config_folder_path=None
):
    if source_kind == "ec":
    ...
    if source_kind == "ic" and input_slices is not None:
        _ensure_source_profiles_1d(source, sibling_sources, input_slices, timenow, config_folder_path)
...
def _ensure_core_sources_placeholders(input_slices, output_ids, param_process, timenow, config_folder_path=None):
    ...
    for source, name in zip(core_sources.source, names):
        _ensure_source_global_quantities(source, timenow)
        if name == "ic":
            _ensure_source_profiles_1d(source, core_sources.source, input_slices, timenow, config_folder_path)
