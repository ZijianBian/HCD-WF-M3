...
ec_waveform_path = 'ec_waveforms.xml'
...
b_fr.grid(row = 1, column = 1,  sticky = 'nwes') #, padx = 3, pady = 3)
...
def edit_waveform(iant): 
    ...
    configure_waveform_parameters = ['power_launched_time','power_launched']
    
    rrow_t = 0
    rrow_r = 0
    for param in ec_dict[iant]:
      
        if param not in configure_waveform_parameters:
            Label(rfr, text = param, bg = c2, anchor = W, justify = LEFT).grid(row = rrow_r, column = 0, sticky = 'ew', padx = 5, pady = 2)
            entrystring = StringVar()
            entrystring.set(ec_dict[iant][param])
            entrystring.trace('w', lambda name, index, mode, param = param, entrystring = entrystring, iant = iant: print(param))
            Entry(rfr, textvariable = entrystring,  bg = c1).grid(row = rrow_r, column =1, sticky = 'ew', padx = 5, pady = 2)
            rrow_r +=1

   
    ## power launched time: 
    powl = ec_dict[iant]['power_launched'].split()
    powl_time = ec_dict[iant]['power_launched_time'].split()
    
  
    Label(lfr, text = 'power \n launched', bg = c3,).grid(row  = 1, column = 2)
    Label(lfr, text = 'time', bg = c3).grid(row = 1, column = 1)

    textwidget_powl_time= Text(lfr, height = len(powl_time), width = 6, bg = c1, padx = 3, pady =2)
    textwidget_powl_time.grid(row = 2, column = 1, sticky = 'nse', padx = (5, 1))
    textwidget_powl_time.bind('<Leave>', lambda event, entryelem = 'power_launched_time', iant = iant: update_ec_dict(iant, textwidget_powl_time.get('1.0', END).replace('\n', ' '), entryelem))


    textwidget_powl = Text(lfr, width = 6, padx = 3, bg = c1)
    textwidget_powl.grid(row = 2, column = 2, sticky = 'nsw', padx = (1,5))
    textwidget_powl.bind('<Leave>', lambda event, entryelem = 'power_launched', iant = iant: update_ec_dict(iant, textwidget_powl.get('1.0', END).replace('\n', ' '), entryelem))
    ...
    def plot_waveform(): 
        powl_time_int = [float(i.strip()) for i in ec_dict[iant]['power_launched_time'].split()]
        powl_int = [float(i.strip()) for i in ec_dict[iant]['power_launched'].split()]
