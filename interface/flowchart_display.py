...
from flowchart_functions import merge_bottom_lines, mergefr_draw_lines, \
    hide_display, click_actors, click_labels, connect_labels
...
def make_flowchart(removed_by_close_button,window,maindict,workflow_param):
    ...
    # --------------------------------------
    initla  = tkinter.Label(base,bg=col.c2,text='INITIALIZING',\
                            font='14',relief='solid',pady=5,width=25)
    initfr  = tkinter.Canvas(base,bg=col.c1,height=1)
    hcdla   = tkinter.Label(base,bg=col.c2,text='H&CD',font='14',relief='solid', pady=5)
    ...
    corela  = tkinter.Label(base,bg=col.c2,text='MAKE CORE IDS',font='14',relief='solid',pady=5)
    corefr  = tkinter.Canvas(base,bg=col.c1,height=1)
    tcontla = tkinter.Label(base,bg=col.c2,text='TIMELOOP CONTROL',font='14',relief='solid',pady=0)
    tcontfr = tkinter.Canvas(base, bg=col.c1, height=1)
    finalla = tkinter.Label(base, bg=col.c2, text='FINALISING',font='14',relief='solid',pady=5)
    ...
    il = tkinter.Label(initfr,text='reading input from database\n- shot:   '\
                       + workflow_param[wfp_ref]['shot_nr']+  '\n- run:   '\
                       + workflow_param[wfp_ref]['run_in']+'\n- machine:   '\
                       + workflow_param[wfp_ref]['input_database']+' \n- start time:   '\
                       + workflow_param[wfp_ref]['tbegin']+'s \n- timestep:   '\
                       + workflow_param[wfp_ref]['dt_required']+'s \n- end time:   '\
                       + workflow_param[wfp_ref]['tend']+'s',\
        ...
        if sum([int(workflow_param[cod_ref][i]) for i in maindict[actors_ref][hsys]]) is not 0:
            ...
            for cat in maindict[actors_ref][hsys]:
                ...
                if int(workflow_param[cod_ref][cat]) is not 0:
                    ...
                    l1 = tkinter.Label(hcdfr,text=' - '+input_text,bg='ivory',\
                    ...
                    l2 = tkinter.Label(hcdfr,text=' - '+output_text, bg='floral white',\
                                       anchor=tkinter.W,justify=tkinter.LEFT)
                    l2.grid(column=ccol,row=rrow+3,sticky='new')
                    l2.grid_remove()
                    
                    l0.bind('<Button-1>',lambda event,l0=l0,l1=l1,l2=l2: \
                            click_actors(l0,l1,l2, conn_list_hcd, hcdfr, base, conn_list))
    ...
    for hsys in maindict[actors_ref]:
        for cat in maindict[actors_ref][hsys]:
            ...
            if((int(workflow_param[cod_ref][cat]) is not 0) and (curval.find('iccoup') is -1)):
                if ''.join(maindict[actors_ref][hsys][cat][curval][1]).find('distributions') \
                   is not -1:
                    merge_distributions += 1
                    
                if ''.join(maindict[actors_ref][hsys][cat][curval][1]).find('distribution_sources')\
                   is not -1:
                    merge_distribution_sources += 1
                    
                if ''.join(maindict[actors_ref][hsys][cat][curval][1]).find('waves')\
                   is not -1:
                    merge_waves += 1
    ...
    if merge_waves > 1:
        mwave = tkinter.Label(labelrow,text='merge waves',bg=col.c4,relief='solid',padx=2,pady=4)
    ...
    if merge_distributions > 1:
        mdist = tkinter.Label(labelrow,text='merge distributions',bg=col.c4,relief='solid',\
                              padx=2,pady=4)
    ...
    if merge_distribution_sources > 1:
        msour = tkinter.Label(labelrow,text='merge sources',bg=col.c4,relief='solid',\
                              padx=2,pady=4)
            ...
            for cat in maindict[make_core_ids_ref][hsys]:
                if int(workflow_param[cod_ref][cat]) is not 0:
                    curval = list(maindict[make_core_ids_ref][hsys][cat])\
                             [int(workflow_param[cod_ref][cat])-1]
                    input_text = "\n - ".join([strv for strv in \
                                               maindict[make_core_ids_ref][hsys][cat][curval][0]])
                    output_text = "\n - ".join([strv for strv in \
                                                maindict[make_core_ids_ref][hsys][cat][curval][1]])

                    corefr.rowconfigure(rrow, minsize=25)

                    l1 = tkinter.Label(corefr, text=input_text, bg=col.c1, relief='solid')
                    ...
                    l2 = tkinter.Label(corefr, text=output_text, bg=col.c1, relief='solid')
                    ...
                    l0.bind('<Button-1>', lambda event, l0=l0, l1= l1, l2=l2: \
                            click_actors(l0,l1,l2, conn_list_core, corefr, base, conn_list))
    ...
    if len(conn_list_core) ==0 :
        ...
        l = tkinter.Label(corefr, text='no make core ids actors selected', \
                          bg=infolabelcolor, padx=4, pady=4)
    ...
    tla = tkinter.Label(tcontfr,text=\
    ...
    il = tkinter.Label(finalfr, text=
               'saving output to database\n- output run nr:   '\
                       +workflow_param[wfp_ref]['run_out'], bg=infolabelcolor, padx=4, pady=4)
    il.pack(side=tkinter.TOP)
