...
def make_frame(hsys,fr_top,actor_name,previous_frame,cp_top,c1,c2,current_config_folder,default):

    # WINDOW CONFIGURATION
    canvas   = tkinter.Canvas(cp_top,borderwidth=0,highlightthickness=0,background=c1)
    frame    = tkinter.Frame(canvas,width=500,height=1500,bg=c1)
    ...
    tkinter.Button(fr_top, text='Restore default', bg=c2, command=lambda:
           make_frame(hsys,fr_top,actor_name,frame,cp_top,c1,c2,current_config_folder,True))\
